...
bool processline(String message) {
  if (message.length() != 0) {
    ...
    if (message == "OK" || message == "ERROR" || message == "ready"|| message == "FAIL") {
      return false;
    }
    //--------------------------------
  }
...
void sendit(const char* message, int timeout = 10000) {
...
//  sendit("AT+CWJAP=\"SKYPNVF2\",\"FD3c4LEP3EAk\"");
//  sendit("AT+CWJAP=\"Ben\'s iPhone\",\"aaabbbccc1\"", 20000);
